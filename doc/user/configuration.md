...
: The optional ''activeScreenOnly'' modifier restricts the action to only execute when the currently active screen matches one of the target screens. This allows creating conditional hotkeys that only trigger when focus is on specific clients. **Important limitation:** When the active screen does not match any of the target screens, the action is not performed, but the original hotkey is consumed by the OS-level hotkey registration and will not function in applications. For example: `keystroke(Control+right,macbookair.lan,activeScreenOnly)` will remap `Control+right` when the active screen is `macbookair.lan`, but on other screens the `Control+right` hotkey will be consumed and do nothing. This is due to OS-level hotkey registration which cannot be bypassed.
